...
password = 'sparta'
cxn_str = f'mongodb+srv://test:{password}@cluster0.z9fd051.mongodb.net/?retryWrites=true&w=majority&appName=AtlasApp'
...
def detail(keyword):
    ...
    if not definitions:
        ...
        'error',
        msg=f'Your word, {keyword}, Could not find'
    ))
    if type(definitions[0]) is str:
        suggestions = ', '.join(definitions)
        return redirect(url_for(
        'error',
        msg=f'Your Word, "{keyword}", Could not find Here are some suggested words {suggestions}?'
    ))
